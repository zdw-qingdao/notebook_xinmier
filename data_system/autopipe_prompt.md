...
1，监督模型的设计与实现；
  ...
  2，只有两个模型推理和对比；
  方案1，通过block来实现
    1，yolo推理block  sam3推理block  结果对比block与数据保存block；
    2，保存数据再进行自动标注与人工检查；手动进行模型训练，workflow替换，模型下载；
  方案2，单独做一栏模型监督；
  先采用方案2，这样比较容易实现，通过block实现后续再考虑，目前会将问题搞的复杂；
  模型监督栏：
    指定运行时间间隔；
    指定相机，执行被监督模型和监督模型，目前对比方法是默认的框对比；只支持detection模型；
    1，取图方式：
      1，间隔指定时间后，循环对每路相机建立连接，取一张图，进行判断；
      2，取检测结果的方式：
        1，相机pub出来
        2，服务器运行，先采用服务器运行的方式；
