...
task_manager_3  每个文件的代码理解，理解整个过程


1, 代码调整：
  1, 去掉这个前缀 tm3- 制定固定的前缀；
  2，factory_modules 添加adapter的路径；通过代码实现，而不是通过配置文件的方式；


目前通过 load_factory_modules 载入需要的文件，

1，通过大模型来总结
  1，通过流程图说明作用；只调整当前版本，后续有需要再增加新的功能；
    说明持久化的方式；
  0，持久化是怎么实现的，理解这个过程；

0, review所有改动，推到main分支，基于这个分支来进行后续的调整；
...
6，数据平台json文件适配的问题，看是否每一部分都进行了适配；华煜 linli fangsong 林立 需要确定责任人；
  这个目前不需要了，后续以2.0为主，之前的平台作为一个临时方案；

3，haowei：
  3，多机多卡测试；
  2，配合前段进行任务情况显示；
  1，先进行迁移的工作；
  逐步迁移；并进行测试；每个迁移review；
  1，sam3/模型推理调用的逻辑：
